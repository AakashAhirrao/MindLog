...
public class SqlJournalService implements JournalService{
    ...
    public void addEntry(Thought thought){
        String sql = "INSERT INTO thoughts (user_id, category, content) VALUES (?, ?, ?)";

        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)){

            stmt.setInt(1, thought.getUserId());
            stmt.setString(2, thought.getCategory().name());
            stmt.setString(3, thought.getContent());
...
//            System.out.println("Thought saved to MySQL! ");
    ...
    @Override
    public List<Thought> getEntriesByCategory(Category category) {
    ...
    @Override
    public List<Thought> searchEntries(String keyword) {
    ...
    @Override
    public void updateEntry(int id, String newContent) {
    ...
    @Override
    public List<Thought> getPagedEntries(int userId, int limit, int offset) {
        List<Thought> results = new ArrayList<>();
        // LIMIT = amount to fetch | OFFSET = amount to skip
        String sql = "SELECT * FROM thoughts WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                // 1. We create the object and name it 't'
                Thought t = new Thought(
                        rs.getInt("id"),
                        Category.valueOf(rs.getString("category").toUpperCase()),
                        rs.getString("content"),
                ...
                // 2. NOW the computer knows who 't' is, so we can set the ID!
